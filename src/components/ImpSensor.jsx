...
const ImpSensor = () => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        for (const row of rows) {
          const { estado } = row;  // Extraemos el campo estado
          const sensor = { estado, id_bote: null };  // id_bote es null por defecto
          await axios.post('http://localhost:3000/api/importarsensor', sensor);
        }
  ...
  return (
    <div style={{ maxWidth: '400px', margin: 'auto', textAlign: 'center', padding: '20px', border: '1px solid #ddd', borderRadius: '10px', boxShadow: '0px 4px 6px rgba(0, 0, 0, 0.1)' }}>
      <h2 style={{ color: '#333' }}>Cargar Sensores desde Excel</h2>
      ...
      <button onClick={() => navigate('/sensor')} style={{ padding: '10px 15px', margin: '5px', backgroundColor: '#007bff', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>
        Cancelar
      </button>
    </div>
  );
