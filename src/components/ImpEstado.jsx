...
const ImpEstado = () => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        for (const row of rows) {
          const { nombre } = row;
          const estado = { nombre, id_pais: null };  // Se asigna null a id_pais inicialmente
          await axios.post('http://localhost:3000/api/importarestado', estado);
        }
  ...
  return (
    <div style={{ maxWidth: '400px', margin: 'auto', textAlign: 'center', padding: '20px', border: '1px solid #ddd', borderRadius: '10px', boxShadow: '0px 4px 6px rgba(0, 0, 0, 0.1)' }}>
      <h2 style={{ color: '#333' }}>Cargar Estados desde Excel</h2>
      ...
      <button onClick={() => navigate('/estado')} style={{ padding: '10px 15px', margin: '5px', backgroundColor: '#007bff', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>
        Cancelar
      </button>
    </div>
  );
