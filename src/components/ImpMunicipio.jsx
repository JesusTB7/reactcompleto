...
const ImpMunicipio = () => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        for (const row of rows) {
          const { nombre } = row;
          const municipio = { nombre, id_estado: null };  // Se asigna null a id_estado inicialmente
          await axios.post('http://localhost:3000/api/importarmunicipio', municipio);
        }
  ...
  return (
    <div style={{ maxWidth: '400px', margin: 'auto', textAlign: 'center', padding: '20px', border: '1px solid #ddd', borderRadius: '10px', boxShadow: '0px 4px 6px rgba(0, 0, 0, 0.1)' }}>
      <h2 style={{ color: '#333' }}>Cargar Municipios desde Excel</h2>
      ...
      <button onClick={() => navigate('/municipio')} style={{ padding: '10px 15px', margin: '5px', backgroundColor: '#007bff', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>
        Cancelar
      </button>
    </div>
  );
