    ...
    return (
        <div style={{ backgroundColor: "#ffffff", height: "100vh", color: "black", display: "flex", flexDirection: "column", alignItems: "center" }}>
            <motion.div
                ...
                style={{ position: "fixed", top: 0, width: "90%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "15px 20px", backgroundColor: "#f0f0f0", boxShadow: "0px 4px 10px rgba(0,0,0,0.3)", zIndex: 1000, borderRadius: "10px", marginTop: "10px" }}
            >
                ...
                <div style={{ display: "flex", gap: "20px" }}>
                    {menuItems.map((item, index) => (
                        <motion.button key={index} onClick={() => navigate(item.route)} whileHover={{ scale: 1.1, color: "#0077b6" }} whileTap={{ scale: 0.9 }} style={{ backgroundColor: "transparent", color: "black", border: "none", cursor: "pointer", textAlign: "center", padding: "10px", fontSize: "16px", fontWeight: "500", transition: "color 0.3s ease" }}>{item.name}</motion.button>
                    ))}
                    <motion.button onClick={handleLogout} whileHover={{ scale: 1.1, backgroundColor: "#d32f2f" }} whileTap={{ scale: 0.9 }} animate={{ opacity: isLoggingOut ? 0.5 : 1 }} style={{ backgroundColor: "#f44336", color: "white", padding: "10px", border: "none", cursor: "pointer", fontSize: "16px", borderRadius: "5px", fontWeight: "500", transition: "background-color 0.3s ease" }} disabled={isLoggingOut}>{isLoggingOut ? "Cerrando..." : "Cerrar sesión"}</motion.button>
                </div>
            </motion.div>
                            ...
                            {botes.length > 0 ? (
                                botes.map((bote) => (
                                    <tr key={bote.id_bote}>
                                        ...
                                        <td><motion.div style={{ width: "30px", height: "30px", borderRadius: "50%", backgroundColor: bote.estado_sensor === "Activo" ? "green" : "red", transition: "background-color 0.5s ease" }} /></td>
                                    </tr>
                                ))
                            ) : (
